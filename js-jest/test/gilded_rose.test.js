...
describe("Gilded Rose with items", () => {
  ...
  it("should not increase quality of Aged Brie over 50", () => {
    const items = [new Item("Aged Brie", 0, 52)];
    gildedRose = new Shop(items);
    gildedRose.updateQuality();
...
it("the quality of a conjured item can't be lower than 0", () => {
  const items = [new Item("Conjured Mana Cake", 0, 0)];
  gildedRose = new Shop(items);
  gildedRose.updateQuality();
  expect(gildedRose.items[0].quality).toBe(0);
  expect(gildedRose.items[0].sellIn).toBe(-1);
